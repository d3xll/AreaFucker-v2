...
local Window = Library.CreateLib("AreaFucker | Juggernaut", "Synapse")
----PLAYERMENU
...
ESPSection:NewToggle("Juggernaut ESP", "The ability to see killers through walls!", function(state)
    if state then
    jugesp = true
    while jugesp == true do
    local function esp(player)
        local b = Instance.new("BoxHandleAdornment", player)
        b.AlwaysOnTop = true
        b.ZIndex = 5
...
MiscAmmo:NewToggle("InfAmmo", "These bullets will never run out!", function(state)
    if state then
        ...
        while infammo == true do
            game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
            game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.CanCollide = false
            game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.Transparency = 1
            game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.Texture.Transparency = 1
            wait()
        end
    else
        infammo = false
        game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.Transparency = 0
        game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.Texture.Transparency = 0
        game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.CFrame = oldpos
        game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].Box.CanCollide = true
    end
end)

MiscAmmo:NewButton("+Ammo", "Refill your ammo!", function()
    oldpos1 = game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].CFrame
	game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
	game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].CanCollide = false
	wait()
	game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].CFrame = oldpos1
	game:GetService("Workspace")["Ammo Bags"]["Box of Shells"].CanCollide = true
end)
...
MiscHitboxes:NewToggle("Players hitbox", "Increase the hitbox of the head by 15 times!", function(state)
    if state then
        ...
        if v.Name ~= game:GetService('Players').LocalPlayer.Name then
        pcall(function()
        v.Character.HumanoidRootPart.Size = Vector3.new(_G.HeadSize,_G.HeadSize,1)
